...
(function($) {
	$.fn.slide = function(options) {
		...
		function toLast(move) {
			switch (move) {
				case 'last':
					...
					el.changeSlide(settings, -(el.children().length-1) * 100);
		...
		$("<ul style='width:" + ($(settings.slideContainer).length * 22 + ($(settings.slideContainer).length - 1) * 5) + "px;margin-left:-" + ($(settings.slideContainer).length * 22 + (($(settings.slideContainer).length - 1) * 5)) / 2 + "px' class='navListBox'>" + navlist + "</ul>").appendTo(el.parent());
