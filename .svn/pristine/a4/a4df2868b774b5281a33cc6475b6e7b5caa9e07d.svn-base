...
(function (root, factory) {
			...
			loadjs: function() {
				...
				return function(url, callback){
					var head = document.getElementsByTagName('head')[0];
						...
						var	isLoaded = document.getElementById('loaded')
	...
	function iShare(options) {
		var defaults = {
					...
					image       : Util.getimg(),
					sites       : ['iShare_weibo','iShare_qq','iShare_tencent','iShare_douban','iShare_qzone','iShare_renren','iShare_youdaonote','iShare_facebook','iShare_linkedin','iShare_twitter','iShare_googleplus','iShare_tumblr','iShare_pinterest'],
					initialized : true,
					isTitle     : true,
					isAbroad    : false,
					WXoptions   : {
		...
		if((this.config.isAbroad === void(0))){
			...
			if(this.config.isAbroad){
				this.config.sites = defaults.sites.slice(7); // 国外
			} else {
				this.config.sites = defaults.sites.slice(0, 7); // 国内
			}
		}
