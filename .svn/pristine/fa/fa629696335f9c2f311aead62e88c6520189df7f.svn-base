...
public class AccountManagerController extends BaseController{
	...
	public ModelAndView showIODetails(HttpSession session,Integer page,String startTime,String endTime,String logId){
		if(page==null){
			page = 1;
		}
		Integer pageSize = 10;
...
//		String logId = this.retrievalCurrentUserInfo(session).getLogId();
	...
	public void tixianForGZH(HttpServletResponse response,HttpSession session, String logId, String money ,String openId, String isWeiXin){
		JsonResult jr = new JsonResult();
		try {
			//获取用户
			LogInfo logInfo = this.retrievalCurrentUserInfo(session);
			if(logInfo != null && "1".equals(isWeiXin)){
				jr = accountPaymentDetailsService.withDraw(logInfo, money);
