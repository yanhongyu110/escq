      ...
      //上传图片
        ...
        $(function () {
        	//取消实名认证，在服务商认证中添加实名信息 js验证
        	$('.piRealname').blur(function(){
        		var tt=$(this); 
        		if(tt.val()!=''&&tt.val()!=null){
          			tt.parents('ul').find('.tips').css({"display":"none"});
    	   		  }else{
    	   			tt.parents('ul').find('.tips').val("真实姓名不能为空");
    	   			tt.parents('ul').find('.tips').show();
    	   		  }
        	})
            ...
            var timeout=function(){
            	 $(".mes_bc:contains('提交')").attr("disabled","false");
            }
             //提交
             $(".mes_bc:contains('提交')").click(function (){
            	
            	 $(".mes_bc:contains('提交')").attr("disabled","true");
             ...
             var temp={};
           	 try{
                 ...
                 if(validate){
                  $.ajax({
                      type : "POST",
                      url : path+"/checkinfo/providerCheck/addCheckInfo.html",
                      data :temp,
