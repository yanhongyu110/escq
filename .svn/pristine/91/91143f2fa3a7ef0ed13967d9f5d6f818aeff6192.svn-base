...
;(function (factory) {
    ...
    $.fn.citys = function(parameter,getApi) {
        ...
        var defaults = {
            dataUrl:path+'/js/citys.json',     //数据库地址
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    var format = {
                        province:function(){
                            $province.empty();
                            $province.append('<li style="font-weight:bold;"><a>全部省份：</a></li>');

                            if(!options.required){
                                $province.append('<li  '+ (!options.province?'class="sv_ms"':'')+'><a>全部</a></li>');
                            }
                            for(i in province){
                                $province.append('<li  '+(i==options.province?'class="sv_ms"':'')+'  ><a>'+province[i]+'</a></li>');
                            }            
                            if(options.province){
                        ...
                        city:function(){
                            $city.empty();
                            $city.append('<li style="font-weight:bold;"><a>级联市级：</a></li>');
                            if(!options.required){
                                $city.append('<li class="sv_ms"><a>全部</a></li>');
                            }
                        ...
                        area:function(){
                            ...
                            if(!hasCity){
                                ...
                                $area.append('<li style="font-weight:bold;"><a>小区街道：</a></li>');
                                $area.css('display','');
                                if(!options.required){
                                    $area.append('<li class="sv_ms"><a>全部</a></li>');
                                }
                                if(options.nodata=='disabled'){
                    ...
                    //事件绑定
                    $province.on('click','li a',function(){
                        $(this).parent('li').siblings().removeClass('sv_ms');
                        options.province = $(this).text();
                        $(this).parent('li').addClass('sv_ms');
                        options.city = 0;
                        options.area = 0;
                        updateData();
                        format.city();
