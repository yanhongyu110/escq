...
<script type="text/javascript" src="${webtxc }/myjs/wap/userinfo/demandListforhomepage.js?v=1245"></script>
			...
			<div class="content have_bg">
				<div class="expert_bg">
					<div class="expert_tx" style="position: relative">
						...
						<h3>${info.logName } </h3>
						<div class="fens"><span>关注 ${countFollow }人</span><span>粉丝  ${count}个</span></div>
						<h4>联系Ta ${info.logCell }</h4>
					...
					<div class="_yue_bottom_left" style="width: 100%
						margin-top: 10px;
						padding: 8px 20px;
						text-align: center;
						background: #f40;
				...
				<%-- <div class="expert_lan">
					<ul>
					    <li><img src="${webtxc }/images/wap/myd.png" align="left"><span>粉丝</span><b>234人</b></li>
...
<%-- 				<div class="expert_jl">
				...
				<div class="expert_zp" id="service_list">
					...
					<c:forEach var="ss" items="${list}" varStatus="vs">
						<div class="tt_zhu" onclick="toDeals('${ss.demId}')" >
							<c:if test="${empty ss.demImage }">
								<div class="tt_zhu_img"><img  src="/images/wap/xq_picture.jpg"></div>
							</c:if>
							<c:if test="${not empty ss.demImage }">
								<div class="tt_zhu_img"><img  src="${ss.demImage }"></div>
							</c:if>
							<h3 class="serviceTitle">${ss.demTitle }</h3> 
						</div>
					</c:forEach>
				</div>
				<br>
