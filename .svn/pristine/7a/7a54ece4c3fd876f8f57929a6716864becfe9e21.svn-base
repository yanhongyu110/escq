...
function selectCommonAjax(pageNo){
	$.ajax({
			type : "post",
			url : path+'/goodsinfo/ProviderInfo/classifyProviderByAjax.html',
			dataType:'json',
			data:{
				'order':order,
				'province':province,
...
function showDiv(res,pageNo){
	for(var i in res){
		...
		var inner = '<div class="sv_zj">'
			...
			+'<p><a><font>'+(piDealnum||0)+'</font>人约见</a> <a><font>'+(piMeetnum||0)+'</font>人想见</a> <a>评分：<font>'+(piRek||0.0)+'</font>分</a></p>'
			+'</div>'
			+'<div class="sv_zj_btn">'
