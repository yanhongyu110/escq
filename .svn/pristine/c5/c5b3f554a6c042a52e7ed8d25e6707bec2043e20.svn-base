...
//		System.out.println(JSON.toJSONString(user, true));
	...
	public ModelAndView jumpmodifyPass(HttpSession session) {
		ModelAndView mv = new ModelAndView();
		LogInfo logInfo = retrievalCurrentUserInfo(session);
//		if (logInfo != null) {
//			String mode = logInfo.getLogMode();
//			//判断用户的登录方式 第三方登录获取电话号码
//			if(StringUtil.isNotEmpty(mode)){
//				ThirdPartyLogin thirdPartyLogin = thirdPartyLoginService.selectLogById(retrievalCurrentUserInfo(session).getLogId());
//				logInfo.setLogCell(thirdPartyLogin.getTelephone());
