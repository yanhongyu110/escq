...
function clearCookie(){
	...
	if (keys) {
		for (var i = keys.length; i--;)
			if(keys[i]!='rmb'&&keys[i]!='logName'&&keys[i]!='logPass')
			document.cookie=keys[i]+'=0;path=/;expires=' + new Date(0).toGMTString();
	}
}
...
function register(){
		...
		if(ua.match(/MicroMessenger/i)=="micromessenger") { 
			var redirect_uri = encodeURIComponent("http://www.jindaoj.com/wap/userinfo/logInfo/jumpRegister.html?isWeiXin=1");
			window.location.href = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxd0c763640d4b8f08&redirect_uri="
				+ redirect_uri
				+ "&response_type=code&scope=snsapi_base&state="
				+ encodeURIComponent("http://www.jindaoj.com/wap/userinfo/logInfo/jumpRegister.html?isWeiXin=1")+"#wechat_redirect";
		} else { 
