        ...
        .click(
            function () {
                ...
                if (checkNull(providerDetails)) {
                    ...
                    $.ajax({
                        url: path + '/wap/userinfo/providerRecommend/addProviderRecommend.html',
                        type: 'post',
                        dataType: 'json',
                        data: {
                    ...
                    editor.loadPlugin('image', function () {
                        editor.plugin.imageDialog({
                            clickFn: function (url, title) {
                                $('#info_icon').val(url);
                                editor.hideDialog();
                                tt.css({'paddingTop':'10px','background-image': 'url("'+url+'")'});
                                tt.children('img,span').remove();
                            },
                        });
                    });
                });
...
function ret() {
    $.ajax({
        url: path + '/wap/userinfo/providerRecommend/modifyStateByLogId.html',
        type: 'post',
        dataType: 'json',
        success: function (jr) {
            if (jr.success) {
