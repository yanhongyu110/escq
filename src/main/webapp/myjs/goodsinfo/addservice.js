...
$(function(){
	...
	$('input[name=psetpoint]').blur(function(){
		...
		if(!checkprice(priceval) && $('input[name=psetpoint] ~ span').length < 1){
			...
			$(this).val(/(\d*?)[].]?/.exec(priceval)[1]);
		}
	});
	//验证见面时长
	$('input[name=servicemeettime]').blur(function(){
		var tlval = $(this).val();
		if(flag != checktl(tlval)){
			flag = checktl(tlval);
		}
	
		if(!flag && $('input[name=servicemeettime] ~ span').length < 1){
			$(this).after('<br><span style="color:#f00;">请按提示正确输入！</span>');
		}
	});
	...
	$('#servicesave').click(function(){
		...
		$.ajax({
			url: path + '/goodsinfo/service/addService.html',
			data:{
	...
	$('input[name=psetprice],input[name=psetpoint]').focus(function(){
		$('.msg').remove();
	});
