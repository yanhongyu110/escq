$(function(){
	$.ajax({
		type: "POST",
		dataType: "json",
		url:path+ "/goodsinfo/service/queryCountRemarkLogIdSrLevel.html",
		data:{
			"logId" : logId
		},
...
function selectCommonAjax(page){
	...
	var rows=3;//每页显示评论条数
		$.ajax({
			type: "POST",
			dataType: "json",
			url:path+ "/goodsinfo/service/queryServiceRemarkByLogIdSrLevel.html",
			data:{
				"logId" : logId,
				"srLevel" : srLevel,
				"rows" : rows,
			...
			success: function (data) {
					for (var i = 0; i < data.list.length; i++) {
						...
						$(".appendcontent1").eq(i).css({"display":"none","padding":"20px 0 0 88px"});
