...
public class HtmlUtil {
    ...
    private static final String regEx_strSpace="&nbsp;";
    /**
     * @param htmlStr
     * @return
     *  删除Html标签
    ...
    public static String delHTMLSpace(String htmlStr) {
        Pattern p_space = Pattern.compile(regEx_space, Pattern.CASE_INSENSITIVE);
        Matcher m_space = p_space.matcher(htmlStr);
        htmlStr = m_space.replaceAll(""); // 过滤空格回车标签
        return htmlStr.trim(); // 返回文本字符串
    }
    public static String delstrSpace(String htmlStr) {
        Pattern p_space = Pattern.compile(regEx_strSpace, Pattern.CASE_INSENSITIVE);
        Matcher m_space = p_space.matcher(htmlStr);
        htmlStr = m_space.replaceAll(""); // 过滤&nbsp;
        return htmlStr.trim(); // 返回文本字符串
    }
